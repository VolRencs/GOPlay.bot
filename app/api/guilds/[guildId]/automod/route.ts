...
export async function GET(_:Request,{params}:{params:Promise<{guildId:string}>}) {
  ...
  const rules=db.prepare("SELECT * FROM automod_rules WHERE guild_id=?").all(guildId) as AutomodRuleRow[];
...
export async function PUT(request:Request,{params}:{params:Promise<{guildId:string}>}) {
  const {guildId}=await params, access=await withGuild(guildId);
  if(access instanceof Response)return access;
  let r:AutomodPutBody;
  try { r=await request.json() as AutomodPutBody; } catch { return NextResponse.json({error:"Некорректный запрос."},{status:400}); }
  ...
  if(typeof r.enabled!=="boolean"||typeof r.escalation!=="boolean")return NextResponse.json({error:"Некорректное состояние правила."},{status:400});
  if(domains!==undefined&&(!Array.isArray(domains)||domains.some(x=>typeof x!=="string"||!x.length||x.length>253)))return NextResponse.json({error:"Некорректный список доменов."},{status:400});
  {
    ...
    const validThresholdShape = typeof r.threshold === "object" && r.threshold !== null && !Array.isArray(r.threshold);
    const validChannels = channels === undefined || isSnowflakeArray(channels);
    if (!validKind || !validActions || !validWindow || !validThresholdShape || !validChannels) {
      return NextResponse.json({error:"Некорректные настройки правила, ролей или канала."},{status:400});
    }
  }
  if(!isValidThreshold(r.threshold))return NextResponse.json({error:"Некорректные числовые пороги правила."},{status:400});
  if(JSON.stringify(r.threshold).length>10000)return NextResponse.json({error:"Слишком большой список порогов."},{status:400});
  const duration=Number(r.threshold.durationSeconds??DEFAULT_TIMEOUT_SECONDS); if(!Number.isFinite(duration)||duration<1||duration>MAX_TIMEOUT_SECONDS)return NextResponse.json({error:"Тайм-аут может длиться от 1 секунды до 28 дней."},{status:400});
