...
import { CardHeader, ConfirmHost, confirmAction, ModalShell, TemplateLibrary, useAsyncAction } from "../../src/components/dashboard/ui.tsx";
...
type AdminGuild = { id: string; name: string; icon: string | null; messages: number; panels: number; events: number };
...
export default function AdminPage() {
  ...
  function refresh() {
    Promise.all([
      ...
      fetch("/api/admin/guilds").then(r => r.ok ? r.json() : Promise.reject()),
    ]).then(([o, g]) => { setOverview(o); setGuilds(g.guilds ?? []); }).catch(() => setError("Не удалось загрузить данные."));
  }
  useEffect(() => { refresh(); }, []);
  ...
  const doLeave = (wipe: boolean) => {
    ...
    run(async () => {
      try {
        ...
        setError("Не удалось выполнить выход.");
      }
    });
  };
          ...
          <>
            <article className="card settings-card">
              ...
              <div className="stat-cards">
                <article className="card stat-card"><span>Серверов</span><strong>{num(overview.guilds)}</strong><small>всего у бота</small></article>
                <article className="card stat-card"><span>Сообщения</span><strong>{num(overview.messages7d)}</strong><small>за 7 дней по всем серверам</small></article>
                <article className="card stat-card"><span>Модерация</span><strong>{num(overview.moderation7d)}</strong><small>срабатываний за 7 дней</small></article>
                <article className="card stat-card"><span>Участники</span><strong>{overview.members !== null ? num(overview.members) : "—"}</strong><small>во всех серверах</small></article>
              </div>
            </article>
            <TemplateLibrary
              title="Серверы"
                  ...
                  {guilds.map(g => (
                    <article className="template-item" key={g.id}>
                      <div>
                        ...
                        <p className="muted">{g.messages.toLocaleString("ru-RU")} сообщений · {g.panels} панелей · {g.events} событий · ID {g.id}</p>
                      </div>
