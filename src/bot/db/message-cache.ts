...
// Кап ленивый: enforceMessageCap вызывается при каждом флеше (раз в 5 секунд),
// отдельный prune на горячем MessageCreate не нужен.
...
export function flushMessageCache(): void {
  ...
  try {
    ...
    flushFailures = logger.warnEvery(flushFailures, 60, "[CACHE] Не удалось слить кэш сообщений — повторю позже", error);
    return;
  }
  flushFailures = 0;
...
// обрезает все затронутые гильдии (см. touchedGuilds в flushMessageCache).
