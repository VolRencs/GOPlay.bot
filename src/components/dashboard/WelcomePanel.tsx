...
import { channelOptions, Select, useAsyncAction } from "./ui.tsx";
...
export function WelcomeSettings({ channels, value, onChange, onSave, onUpload, bgTimestamp }: { channels: Channel[]; value: Welcome; onChange: (value: Welcome) => void; onSave: () => unknown; onUpload:(file:File)=>void; bgTimestamp: number }) {
  ...
  return <div className="welcome-layout"><div className="event-settings"><MemberEvent title="Приветствие новых участников" event="welcome" channels={channels} value={value} onChange={onChange} onUpload={onUpload} bgTimestamp={bgTimestamp}/><MemberEvent title="Прощание с участником" event="goodbye" channels={channels} value={value} onChange={onChange} onUpload={onUpload} bgTimestamp={bgTimestamp}/></div><aside className="card variables-card floating-preview"><h2>Переменные</h2><div className="variables-list">{welcomeVariables.map(([token,help])=><span key={token}><code>{token}</code> {help}</span>)}</div><button className="btn" disabled={saving} onClick={()=>run(onSave)}>{saving?"Сохраняем…":"Сохранить"}</button></aside></div>;
}
...
function MemberEvent({title,event,channels,value,onChange,onUpload,bgTimestamp}:{title:string;event:"welcome"|"goodbye";channels:Channel[];value:Welcome;onChange:(value:Welcome)=>void;onUpload:(file:File)=>void;bgTimestamp:number}) {
  ...
  return <section className="card"><div className="section-title"><div><h2>{title}</h2><p className="muted">{goodbye?"Канал и текст прощального сообщения.":"Отдельные канал, текст, фон и персональная картинка."}</p></div><label className="switch"><input type="checkbox" aria-label={goodbye?"Включить прощание":"Включить приветствие"} checked={Boolean(enabled)} onChange={e=>set({enabled:+e.target.checked})}/><span/></label></div><div className="event-grid"><div className="settings-card"><label>Канал<Select value={channel??""} onChange={v=>set({channel:v||null})} ariaLabel="Канал события" options={channelOptions(channels, "Выберите канал")}/></label><label>Текст сообщения<textarea rows={4} maxLength={2000} value={message} onChange={e=>set({message:e.target.value})}/></label>{!goodbye&&<><label className="check-row"><input type="checkbox" checked={Boolean(imageEnabled)} onChange={e=>set({imageEnabled:+e.target.checked})}/> Отправлять персональную картинку</label><div><span className="field-label">Фон картинки</span><label className="file-pick"><input type="file" accept="image/png,image/jpeg,image/webp" onChange={e=>e.target.files?.[0]&&onUpload(e.target.files[0])}/><span className="btn secondary small">Выбрать файл</span></label></div>{Boolean(imageEnabled)&&<ImageEditor value={config} onChange={config=>set({config})}/>}</>}</div>{!goodbye&&<WelcomePreview message={message} config={config} background={background} enabled={Boolean(imageEnabled)} onChange={config=>set({config})}/>}</div></section>;
}
...
function EditorNumber({ label, field, value, min, max, onChange }: { label: string; field: keyof WelcomeImageConfig; value: WelcomeImageConfig; min: number; max: number; onChange: (value: WelcomeImageConfig) => void }) {
  return <label>{label}<input type="number" min={min} max={max} value={value[field] as number} onChange={e => onChange({ ...value, [field]: Number(e.target.value) })}/></label>;
}
